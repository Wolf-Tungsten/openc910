...
#include "Vct_vfalu_top_pipe6.h"
#include "Vct_vfalu_top_pipe6___024root.h"
#include "Vct_vfalu_top_pipe6__Syms.h"
...
void tick(Vct_vfalu_top_pipe6& dut) {
...
int main(int argc, char** argv) {
    ...
    auto run_fadd = [&](const fadd::Scenario& sc, bool check_expected = true) -> bool {
        ...
        dut.dp_vfalu_ex1_pipex_mtvr_src0 = 0;
        dut.dp_vfalu_ex1_pipex_sel = 0b010;  // fadd pipeline
        tick(dut);                           // EX1 -> EX2
        dut.dp_vfalu_ex1_pipex_sel = 0;
        tick(dut);  // EX2 -> EX3
        ...
        tick(dut);  // allow valids to clear
        return true;
    };
    ...
    auto drive_fadd_no_check = [&](uint32_t func, uint64_t s0, uint64_t s1) {
        dut.dp_vfalu_ex1_pipex_func = func;
        dut.dp_vfalu_ex1_pipex_srcf0 = s0;
        dut.dp_vfalu_ex1_pipex_srcf1 = s1;
        dut.dp_vfalu_ex1_pipex_imm0 = 0;
    ...
    auto drive_fadd_no_check_rm = [&](uint32_t func, uint64_t s0, uint64_t s1, uint8_t imm, uint8_t rm) {
        dut.dp_vfalu_ex1_pipex_func = func;
        dut.dp_vfalu_ex1_pipex_srcf0 = s0;
        dut.dp_vfalu_ex1_pipex_srcf1 = s1;
    ...
    std::vector<fadd::Scenario> scenarios = {
    ...
    if (run_fadd({"double_maxnm", fadd::make_func(true, false, false, false, false, true, false, 0), 0xBFF0000000000000ULL, 0x4008000000000000ULL, 0x4008000000000000ULL, 0}) == false)
        return 1;
    if (run_fadd({"single_minnm", fadd::make_func(false, true, false, false, false, false, true, 0), fadd::pack_single(0x7F800000), fadd::pack_single(0x3F000000), fadd::pack_single(0x3F000000), 0}) ==
        false)
        return 1;
    if (run_fadd({"half_maxnm", fadd::make_func(false, false, false, false, false, true, false, 0), fadd::pack_half(0xFC00), fadd::pack_half(0x3C00), fadd::pack_half(0x3C00), 0}) == false)
        return 1;

    if (!run_cmp("cmp_feq_single", fadd::make_func(false, true, false, false, true, false, false, 0b00001), fadd::pack_single(0x3F800000),
                 fadd::pack_single(0x3F800000), 1))
        return 1;
    if (!run_cmp("cmp_flt_double", fadd::make_func(true, false, false, false, true, false, false, 0b00010), 0x3FF0000000000000ULL, 0x4008000000000000ULL,
                 1))
        return 1;
    if (!run_cmp("cmp_fle_half", fadd::make_func(false, false, false, false, true, false, false, 0b00100), fadd::pack_half(0x3800), fadd::pack_half(0x3C00),
                 1))
        return 1;
    if (!run_cmp("cmp_unord_double", fadd::make_func(true, false, false, false, true, false, false, 0b00001), 0x7FF8000000000001ULL, 0x3FF0000000000000ULL,
                 0))
        return 1;
    if (!run_cmp("cmp_unord_single", fadd::make_func(false, true, false, false, true, false, false, 0b00010), fadd::pack_single(0x7FC00001),
                 fadd::pack_single(0x3F800000), 0))
        return 1;
    if (!run_cmp("cmp_unord_half", fadd::make_func(false, false, false, false, true, false, false, 0b00100), fadd::pack_half(0x7E00), fadd::pack_half(0x3C00),
                 0))
        return 1;

    std::vector<fadd::Scenario> rounding = {
        {"double_rtz_add", fadd::make_func(true, false, true, false, false, false, false, 0), 0x3FF0000000000000ULL, 0x3FF0000000000000ULL,
         0x4000000000000000ULL, 0b001},
        {"single_rdn_add", fadd::make_func(false, true, true, false, false, false, false, 0), fadd::pack_single(0x3E99999A),
         fadd::pack_single(0x3E4CCCCD), fadd::pack_single(0x3F000000), 0b010},
        {"single_rup_add", fadd::make_func(false, true, true, false, false, false, false, 0), fadd::pack_single(0x3E99999A),
         fadd::pack_single(0x3E99999A), fadd::pack_single(0x3F19999A), 0b111},
        {"double_rmm_add", fadd::make_func(true, false, true, false, false, false, false, 0), 0x3FF0000000000000ULL, 0x3FF0000000000000ULL,
         0x4000000000000000ULL, 0b111},
    };
    ...
    if (!run_fadd({"single_nan_add", fadd::make_func(false, true, true, false, false, false, false, 0), fadd::pack_single(0x7FC00001),
                   fadd::pack_single(0x3F800000), fadd::pack_single(0x7FC00001), 0})) {
        return 1;
    }
    dut.vfpu_yy_xx_dqnan = 0;
    ...
    auto close_sweep = [&](bool is_single, bool is_half) {
        for (uint16_t frac = 0x0001; frac < 0x0008; ++frac) {
            uint64_t a = is_single ? fadd::pack_single(0x3F800000 + (static_cast<uint32_t>(frac) << 7))
                                   : fadd::pack_half(static_cast<uint16_t>(0x3C00 + frac));
            uint64_t b = is_single ? fadd::pack_single(0x3F800000 - (static_cast<uint32_t>(frac) << 6))
                                   : fadd::pack_half(static_cast<uint16_t>(0x3C00 - frac));
    ...
    for (uint8_t rm_mode = 0; rm_mode < 5; ++rm_mode) {
        for (int i = 0; i < 800; ++i) {
            ...
            drive_fadd_no_check_rm(func_double_sub, src0, src1, 0b111, rm_mode);
        }
    }
    for (uint8_t rm_mode = 0; rm_mode < 5; ++rm_mode) {
        for (int i = 0; i < 400; ++i) {
            ...
            drive_fadd_no_check_rm(func_half_sub, src0, src1, 0b111, rm_mode);
        }
    }
    ...
    for (int i = 0; i < 100000; ++i) {
        ...
        uint32_t func = fadd::make_func(is_double, is_single, (i & 1), (i & 2), (i & 4), (i & 8), (i & 16), static_cast<uint8_t>(i));
        dut.dp_vfalu_ex1_pipex_func = func;
        if (is_double) {
            dut.dp_vfalu_ex1_pipex_srcf0 = next_rand();
            dut.dp_vfalu_ex1_pipex_srcf1 = next_rand();
        } else if (is_single) {
            dut.dp_vfalu_ex1_pipex_srcf0 = fadd::pack_single(static_cast<uint32_t>(next_rand()));
            dut.dp_vfalu_ex1_pipex_srcf1 = fadd::pack_single(static_cast<uint32_t>(next_rand()));
        } else {
            dut.dp_vfalu_ex1_pipex_srcf0 = fadd::pack_half(static_cast<uint16_t>(next_rand()));
            dut.dp_vfalu_ex1_pipex_srcf1 = fadd::pack_half(static_cast<uint16_t>(next_rand()));
        }
        dut.dp_vfalu_ex1_pipex_mtvr_src0 = 0;
        dut.dp_vfalu_ex1_pipex_imm0 = static_cast<uint8_t>(next_rand() & 0x7);
        dut.vfpu_yy_xx_rm = static_cast<uint8_t>(next_rand() & 0x7);
        dut.dp_vfalu_ex1_pipex_sel = 0b010;
        tick(dut);
        dut.dp_vfalu_ex1_pipex_sel = 0;
        tick(dut);
    ...
    auto run_fspu = [&](const fspu::Scenario& sc) -> bool {
        ...
        dut.dp_vfalu_ex1_pipex_imm0 = 0;
        dut.vfpu_yy_xx_rm = 0;
        dut.dp_vfalu_ex1_pipex_sel = 0b001;  // fspu pipeline
        tick(dut);
        const uint64_t mfvr = dut.pipex_dp_ex1_vfalu_mfvr_data;
        dut.dp_vfalu_ex1_pipex_sel = 0;
        tick(dut);
    ...
    std::vector<fspu::Scenario> fspu_scenarios = {
        ...
        {"single_fsgnjn", fspu::build_func(false, true, false, false, false, false, true, false), fspu::pack_single_val(0x3F800000u),
         fspu::pack_single_val(0xBF800000u), 0},
        {"single_fsgnjx", fspu::build_func(false, true, false, false, false, false, false, true), fspu::pack_single_val(0xBF800000u),
         fspu::pack_single_val(0xBF800000u), 0},
        {"single_fsgnj", fspu::build_func(false, true, false, false, false, true, false, false), fspu::pack_single_val(0xBF800000u),
         fspu::pack_single_val(0x3F800000u), 0},
        ...
        {"half_fsgnjn", fspu::build_func(false, false, false, false, false, false, true, false), fspu::pack_half_val(0x3C00u),
         fspu::pack_half_val(0xBC00u), 0},
        {"half_fsgnjx", fspu::build_func(false, false, false, false, false, false, false, true), fspu::pack_half_val(0x3C00u),
         fspu::pack_half_val(0xBC00u), 0},
    };
    ...
    for (auto v : double_vals) {
        drive_fspu_no_check(fspu::build_func(true, false, true, false, false, false, false, false), v, 0, 0);
    }
    for (auto v : single_vals) {
        drive_fspu_no_check(fspu::build_func(false, true, true, false, false, false, false, false), fspu::pack_single_val(v), 0, 0);
    }
    for (auto v : half_vals) {
        drive_fspu_no_check(fspu::build_func(false, false, true, false, false, false, false, false), fspu::pack_half_val(v), 0, 0);
    }
    ...
    uint64_t lcg2 = 13;
    auto next_rand2 = [&]() {
        lcg2 = lcg2 * 1103515245ULL + 12345ULL;
        return lcg2;
    };
    for (int i = 0; i < 20000; ++i) {
        dut.dp_vfalu_ex1_pipex_func = static_cast<uint32_t>(next_rand2());
        dut.dp_vfalu_ex1_pipex_srcf0 = next_rand2();
        dut.dp_vfalu_ex1_pipex_srcf1 = next_rand2();
        dut.dp_vfalu_ex1_pipex_mtvr_src0 = next_rand2();
        dut.dp_vfalu_ex1_pipex_imm0 = 0;
        dut.dp_vfalu_ex1_pipex_sel = 0b001;
        tick(dut);
        dut.dp_vfalu_ex1_pipex_sel = 0;
    ...
    if (!cov_out) {
        cov_out = "build/case031/coverage.dat";
    }
