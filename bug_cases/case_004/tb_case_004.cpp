...
int main(int argc, char **argv) {
    ...
    if (dut->retire0 != 1 || dut->retire1 != 0 || dut->retire2 != 0) {
        std::cerr << "[case_004] retire signals mismatch after first tick" << std::endl;
        return 1;
    }
    if (dut->axi_awlen != 2 || dut->axi_mem_addr != 8) {
        std::cerr << "[case_004] axi signals mismatch after first tick" << std::endl;
        return 1;
    }
    if (dut->biu_pad_wvalid != wvalid_expected || dut->biu_pad_wstrb != expected_wstrb) {
        std::cerr << "[case_004] biu_pad_w signals mismatch after first tick" << std::endl;
        return 1;
    }
    if (!wide128_equals(dut->biu_pad_wdata, expected_wdata)) {
        std::cerr << "[case_004] biu_pad_wdata mismatch after first tick" << std::endl;
        return 1;
    }
    ...
    if (dut->retire0 != 0 || dut->retire1 != 1 || dut->retire2 != 0) {
        std::cerr << "[case_004] retire signals mismatch after second tick" << std::endl;
        return 1;
    }
    if (dut->value0 != 4 || dut->value1 != 7 || dut->value2 != 8) {
        std::cerr << "[case_004] value signals mismatch after second tick" << std::endl;
        return 1;
    }
    if (dut->axi_awlen != 4 || dut->axi_mem_addr != 16) {
        std::cerr << "[case_004] axi signals mismatch after second tick" << std::endl;
        return 1;
    }
